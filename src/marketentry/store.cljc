(ns marketentry.store
  "SSoT for the JAM market-entry compliance actor, behind a `Store`
...
(defn demo-data
  ...
  eng-1 clean (above the J$1,500,000 PPC-registration threshold, and
  registered), eng-2 unknown-jurisdiction (no spec-basis), eng-3
  fee-mismatch, eng-4 missing-PPC-registration (above threshold, NOT
  registered -- the flagship violation), eng-5 missing-Companies-
  Office-registration-verification, eng-6 a second clean case BELOW
  the J$1,500,000 threshold (so `:requires-ppc-registration?` is
  correctly false -- demonstrates the threshold rule both ways) --
  mirroring cloud-itonami-iso3166-ago's 6-engagement demo shape.

  `:tender-value-jmd` is the value (in Jamaican dollars) of the
  Government of Jamaica contract the operator intends to tender on --
  NOT this actor's own engagement fee. It is documentary/demo
  grounding for how `:requires-ppc-registration?` was set at intake
  (per `marketentry.facts/registration-threshold-jmd`); the governor
  itself (same conditional-trust architecture as every sibling actor's
  flagship check) trusts `:requires-ppc-registration?` and
  independently verifies only `:has-ppc-registration?`."
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Kita Systems JA" :portal "PPC (ppc.gov.jm)"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :tender-value-jmd 2000000
             :requires-ppc-registration? true :has-ppc-registration? true
             :requires-companies-office-registration? true :companies-office-registration-verified? true
             :drafted? false :submitted? false
             :jurisdiction "JAM" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "PPC (ppc.gov.jm)"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :tender-value-jmd 2000000
             :requires-ppc-registration? true :has-ppc-registration? true
    ...
    "eng-3" {:id "eng-3" :operator "Minami Systems" :portal "PPC (ppc.gov.jm)"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "JAM" :status :intake}
    "eng-4" {:id "eng-4" :operator "Higashi Export" :portal "PPC (ppc.gov.jm)"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :tender-value-jmd 1800000
             :requires-ppc-registration? true :has-ppc-registration? false
             :requires-companies-office-registration? true :companies-office-registration-verified? true
             :drafted? false :submitted? false
             :jurisdiction "JAM" :status :intake}
    "eng-5" {:id "eng-5" :operator "Nishi Logistics" :portal "PPC (ppc.gov.jm)"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :tender-value-jmd 2000000
             :requires-ppc-registration? true :has-ppc-registration? true
             :requires-companies-office-registration? true :companies-office-registration-verified? false
             :drafted? false :submitted? false
             :jurisdiction "JAM" :status :intake}
    "eng-6" {:id "eng-6" :operator "Chuo Civic Tech" :portal "PPC (ppc.gov.jm)"
             :base-fee 400000 :monthly-rate 25000 :monitoring-months 6
             :claimed-fee 550000.0
             :tender-value-jmd 900000
             :requires-ppc-registration? false :has-ppc-registration? false
             :requires-companies-office-registration? true :companies-office-registration-verified? true
             :drafted? false :submitted? false
             :jurisdiction "JAM" :status :intake}}})
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               tender-value-jmd
                               requires-ppc-registration? has-ppc-registration?
                               requires-companies-office-registration? companies-office-registration-verified?
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    tender-value-jmd                      (assoc :engagement/tender-value-jmd tender-value-jmd)
    (some? requires-ppc-registration?)    (assoc :engagement/requires-ppc-registration? requires-ppc-registration?)
    (some? has-ppc-registration?)         (assoc :engagement/has-ppc-registration? has-ppc-registration?)
    (some? requires-companies-office-registration?)
    (assoc :engagement/requires-companies-office-registration? requires-companies-office-registration?)
    (some? companies-office-registration-verified?)
    (assoc :engagement/companies-office-registration-verified? companies-office-registration-verified?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   :engagement/monitoring-months :engagement/claimed-fee :engagement/tender-value-jmd
   :engagement/requires-ppc-registration? :engagement/has-ppc-registration?
   :engagement/requires-companies-office-registration? :engagement/companies-office-registration-verified?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :tender-value-jmd (:engagement/tender-value-jmd m)
     :requires-ppc-registration? (boolean (:engagement/requires-ppc-registration? m))
     :has-ppc-registration? (boolean (:engagement/has-ppc-registration? m))
     :requires-companies-office-registration? (boolean (:engagement/requires-companies-office-registration? m))
     :companies-office-registration-verified? (boolean (:engagement/companies-office-registration-verified? m))
