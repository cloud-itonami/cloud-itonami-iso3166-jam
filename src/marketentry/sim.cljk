(ns marketentry.sim
  ...
  commit), then shows HARD-hold scenarios, including the below-
  threshold engagement that correctly does NOT need PPC registration."
...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (JAM, clean, above J$1.5M threshold) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up ppc-registration-missing) ==")
    ...
    (println "== filing/submit eng-4 (above J$1.5M threshold, PPC registration missing -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up companies-office-registration-unverified) ==")
    ...
    (println "== filing/submit eng-5 (companies-office-registration-unverified -> HARD hold) ==")
    (println (exec-op actor "t11" {:op :filing/submit :subject "eng-5"} operator))

    (println "== jurisdiction/assess eng-6 (below J$1.5M threshold -- does NOT need PPC registration) ==")
    ...
    (println "== filing/submit eng-6 (below threshold, clean -> escalates then commits, NOT a ppc-registration-missing hold) ==")
