  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Kita Systems JA"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Kita Systems JA" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest ppc-registration-missing-is-held-and-unoverridable
  (testing "engagement above the J$1.5M threshold with no PPC registration on file -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:ppc-registration-missing} (-> (store/ledger db) last :basis)))
...
(deftest companies-office-registration-unverified-is-held-and-unoverridable
  (testing "unverified Companies Office of Jamaica registration when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:companies-office-registration-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))

(deftest below-threshold-engagement-does-not-require-ppc-registration
  (testing "an engagement below J$1,500,000 correctly has :requires-ppc-registration? false, so filing/submit is NOT held on ppc-registration-missing even without :has-ppc-registration?"
    (let [[db actor] (fresh)
          e (store/engagement db "eng-6")]
      (is (< (:tender-value-jmd e) 1500000) "eng-6 fixture models a below-threshold GOJ tender value")
      (is (false? (:requires-ppc-registration? e)))
      (is (false? (:has-ppc-registration? e)) "not registered, and correctly does not need to be")
      (let [_ (assess! actor "t8pre" "eng-6")
            _ (draft! actor "t8pre" "eng-6")
            r1 (exec-op actor "t8" {:op :filing/submit :subject "eng-6"} operator)]
        (is (= :interrupted (:status r1)) "still escalates for human approval (submit always does) -- but NOT because of ppc-registration-missing")
