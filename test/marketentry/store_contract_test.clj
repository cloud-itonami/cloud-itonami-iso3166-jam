...
(defn- exercise [s]
  (store/commit-record! s {:effect :engagement/upsert
                           :value {:id "eng-x" :operator "X Co" :jurisdiction "JAM"
                                   :base-fee 100 :monthly-rate 10 :monitoring-months 1
                                   :claimed-fee 110.0 :tender-value-jmd 2000000
                                   :requires-ppc-registration? true :has-ppc-registration? true
                                   :requires-companies-office-registration? true
                                   :companies-office-registration-verified? true
                                   :drafted? false :submitted? false :status :intake}})
  (store/commit-record! s {:effect :assessment/set
                           :path ["eng-x"]
                           :payload {:jurisdiction "JAM" :checklist ["a"] :spec-basis "x"}})
...
(deftest mem-and-datomic-parity
  (let [mem (store/seed-db)
    ...
    (is (= (:tender-value-jmd (:engagement m)) (:tender-value-jmd (:engagement d))))
